...
def generatingInputfromStart(start):
    # load ascii text and covert to lowercase
    filename = "C:/Users/dell/Desktop/btre_project/finalYear/venv/textsequence/lstmFirst/lstm/dataset.txt"
    raw_text = open(filename, 'r', encoding='utf-8').read()
    raw_text = raw_text.lower()
    ...
    reference=''
    for i in range(10):
        pat = dataX[start+100*(i+1)]
        reference = reference + ''.join([int_to_char[value] for value in pat])

    return sequence,reference



def calculateScore(candidate,reference):
